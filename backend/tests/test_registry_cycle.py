...
def test_model_details_and_adapter_catalog(client, owner_headers):
    _bootstrap(client)
    current = client.get("/api/v1/models/current", headers=owner_headers).json()
    details = client.get(f"/api/v1/models/{current['id']}", headers=owner_headers).json()
    assert details["id"] == current["id"]
    assert details["adapter_info"]["key"] == "neuralcore"
    assert isinstance(details["artifacts"], list)

    adapters = client.get("/api/v1/models/adapters", headers=owner_headers).json()
    keys = {a["key"] for a in adapters}
    assert {"neuralcore", "huggingface", "gguf", "unknown"}.issubset(keys)

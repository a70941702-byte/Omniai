...
class Orchestrator:
    ...
    def emergency_halt(self, reason: str = "owner emergency stop") -> dict:
        self._stop.set()
        if self._thread and self._thread.is_alive():
            self._thread.join(timeout=1)
        audit.log("owner", "orchestrator_emergency_halt", {"reason": reason})
        return {"stopped": True, "reason": reason, "thread_alive": bool(self._thread and self._thread.is_alive())}
